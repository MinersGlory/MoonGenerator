...
public class MoonCavePopulator extends BlockPopulator {
            ...
            for(int z=-radius; z<=radius; z++)
                for(int y=-radius/2; y<=radius/2; y++) {
                    ...
                    if(v.distance(d) < radius) {
                        ...
                        if(rel.getType() != Material.AIR && rel.getType() != Material.BEDROCK && rel.getType() != Material.WATER && rel.getType() != Material.WATER) {
        ...
        for(int x=-radius; x<=radius; x++)
            for(int z=-radius; z<=radius; z++) {
                ...
                if(v.distance(d) < radius && rand.nextInt(35) > 9) {
                    rel = block.getRelative(x, y, z);
                    if(rel.getType() != Material.BEDROCK && rel.getType() != Material.WATER && rel.getType() != Material.WATER) {
